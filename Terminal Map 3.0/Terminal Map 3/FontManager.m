...
- (BOOL)loadFontURL:(NSURL *)url {
	...
	CFDictionarySetValue(fonts, (__bridge void *)url, newFont);
...
- (CGFontRef)fontWithName:(NSString *)filename {
	...
	if (url != nil) {
		font = (CGFontRef)CFDictionaryGetValue(fonts,(__bridge void *)url);
	}
	return font;
}
...
- (ZFont *)zFontWithName:(NSString *)filename pointSize:(CGFloat)pointSize {
	...
	if (url != nil) {
		CGFontRef cgFont = (CGFontRef)CFDictionaryGetValue(fonts, (__bridge void *)url);
		if (cgFont != NULL) {
...
- (ZFont *)zFontWithURL:(NSURL *)url pointSize:(CGFloat)pointSize {
	...
	if (cgFont == NULL && [self loadFontURL:url]) {
		cgFont = (CGFontRef)CFDictionaryGetValue(fonts, (__bridge void *)url);
	}
	if (cgFont != NULL) {
		return [ZFont fontWithCGFont:cgFont size:pointSize];
	}
